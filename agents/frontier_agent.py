...
class FrontierAgent(Agent):
    ...
    def __init__(self, collection):
        ...
        """
        setup this instance by connection to the data OpenAI or DeepSeek, to the Chroma DataStore,
        ...
        self.log('Frontier Agent is ready to go')

    def make_context(self, similar: List[str], prices: List[float]) -> str:
        ...
        for similar,price in zip(similar,prices):
    ...
    def find_similars(self, description: str):
        ...
        price = [m['price'] for m in results['metadatas'][0][:]]
    ...
    def price(self, description: str) -> float:
        ...
        documenets, prices = self.find_similars(description)
        self.log(f"Frontier Agent is about to call {self.MODEL} with context including 5 similar products")

        response = self.client.chat.completions.create(
            model=self.MODEL,
            messages=self.message_for(description, documenets, prices),
            seed=42,
            max_tokens=5
        )        
        reply = response.choices[0].message.content
        result = self.get_price(reply)
        self.log(f"Frontier Agent has estimated the price of the product to be ${result:.2f}")
        return result